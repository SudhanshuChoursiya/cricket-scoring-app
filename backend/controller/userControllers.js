...
const createMatchController = asyncHandler(async (req, res) => {
    ...
    const isEmpty = [
        ...
        if (Array.isArray(field)) return field.length !== 11;
        if (typeof field === "string") return field.trim() === "";
        return field === null || field === undefined;
    });

    if (isEmpty) throw new ApiError(400, "Required field cannot be empty");
    if (isNaN(totalOvers) || Number(totalOvers) <= 0)
        throw new ApiError(400, "Total overs should be greater than 0");

    //check tournament status only when linked to a tournament
    if (tournamentId) {
        ...
        if (tournament.status !== "ongoing") {
            throw new ApiError(
                400,
                `Cannot create match , tournament is ${tournament.status}`
            );
        }
    }
...
const createTournamentController = asyncHandler(async (req, res) => {
    ...
    if (!name || !location.city || !location.ground || !startDate || !endDate) {
        throw new ApiError(400, "plz fill all the required field");
    }
...
const updateTournamentController = asyncHandler(async (req, res) => {
    ...
    if (tournament.createdBy.toString() !== user._id.toString()) {
        throw new ApiError(
            403,
    ...
    if (location) {
        ...
        if (location.ground) updatedFields["location.ground"] = location.ground;
    }
...
const updateScoreController = asyncHandler(async (req, res) => {
    ...
    // Helper function to the over and handle over completion
    const endOver = () => {
...
const undoScoreController = asyncHandler(async (req, res) => {
    const matchId = req.params.matchId;
    const { lastAction, previousOverTimeline } = req.body;
    const user = req.user;
...
/*const endMatchController = asyncHandler(async (req, res) => {
  const { isMatchAbandoned, winningTeamId, tournamentId } = req.body;
  console.log(req.body)
  const user = req.user;
  const matchId = req.params.matchId;

  const match = await MatchModel.findOne({ _id: matchId, createdBy: user._id });
  ...
  if (isMatchAbandoned) {
    ...
    if (currentInning.battingTeam.teamId.equals(winningTeamId)) {
      match.matchResult = !match.isSuperOver
        ? {
            status: "Win",
            winningTeam: currentInning.battingTeam.name,
      ...
      match.matchResult = !match.isSuperOver
        ? {
            status: "Win",
            winningTeam: currentInning.bowlingTeam.name,
            winningMargin: `${match.targetScore - match.inning2.totalScore} runs`,
          }
        : {
            status: "Super Over",
            winningTeam: match.superOver?.inning2?.bowlingTeam?.name || "",
  ...
  if (match.isInningChangePending) {
    match.isInningChangePending = false;
  }

  const playersStats = new Map();
  const innings = [match.inning1, match.inning2];

  if (match.isSuperOver && match.superOver) {
    innings.push(match.superOver.inning1, match.superOver.inning2);
  }

  for (const inning of innings) {
    if (!inning) continue;

    for (const player of inning.battingTeam.playing11 || []) {
      if (!player?.playerId) continue;
      const id = player.playerId.toString();

      if (!playersStats.has(id)) {
        playersStats.set(id, {
      ...
      const stats = playersStats.get(id);
    ...
    for (const player of inning.bowlingTeam.playing11 || []) {
      if (!player?.playerId) continue;
      const id = player.playerId.toString();
      ...
      stats.ballsBowled += (player.overs || 0) * 6 + (player.ballsBowled || 0);
      stats.runsConceded += player.runsConceded || 0;
    }
  }

  const finalStats = Array.from(playersStats.values());
  if (finalStats.length > 0) {
    await updatePlayerStats(finalStats, tournamentId);
  }

  io.to(matchId).emit("scoreUpdated", { match });
  await match.save();

  res
    .status(200)
    .json(new ApiResponse(200, match, "Match ended successfully"));
});*/
...
const endMatchController = asyncHandler(async (req, res) => {
        ...
        if (currentInning.battingTeam.teamId.equals(winningTeamId)) {
            match.matchResult = !match.isSuperOver
                ...
                : {
                      status: "Super Over",
                      winningTeam:
                          match.superOver?.inning2?.battingTeam?.name || "",
                      winningMargin: null
                  };
        } else if (currentInning.bowlingTeam.teamId.equals(winningTeamId)) {
            match.matchResult = !match.isSuperOver
                ...
                : {
                      status: "Super Over",
                      winningTeam:
                          match.superOver?.inning2?.bowlingTeam?.name || "",
                      winningMargin: null
                  };
        }
    }

    // Clear pending inning change
    if (match.isInningChangePending) {
        match.isInningChangePending = false;
    }
    ...
    for (const inning of innings) {
        ...
        for (const player of inning.bowlingTeam.playing11 || []) {
            ...
            playersStats.push({
                ...
                ballsBowled:
                    (player.overs || 0) * 6 + (player.ballsBowled || 0),
    ...
    // --- Update All Players in Bulk ---
    if (playersStats.length > 0) {
        await updatePlayerStats(playersStats, tournamentId);
    }
...
const getAllTournamentsController = asyncHandler(async (req, res) => {
    // const user = req.user;
    const tournaments = await TournamentModel.find();

    if (!tournaments) {
        throw new ApiError(404, "no tournament has been found");
    }
...
const getSingleTournamentController = asyncHandler(async (req, res) => {
    const { tournamentId } = req.params;
    ...
    const matches = await MatchModel.find({ tournamentId: tournamentId });

    // Return tournament + matches
    res.status(200).json(
        new ApiResponse(200, {
            ...tournamentDetails.toObject(),
            matches // empty array if no matches exist
        })
    );
});

const getSearchedMatchesController = asyncHandler(async (req, res) => {
...
const getSearchedTournamentsController = asyncHandler(async (req, res) => {
    const searchQuery = req.query.searched?.trim();
    const user = req.user;
    ...
    const query = {
        $or: [
            ...
            { "location.ground": { $regex: searchQuery, $options: "i" } }
        ],
        createdBy: user._id
    };
