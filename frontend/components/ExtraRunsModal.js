...
const ExtraRunsModal = ({ showSpinner, handleUpdateScore }) => {
    ...
    const handleConfirmModal = () => {
        if (!/^\d{1,2}$/.test(payload?.runsInput?.value)) {
            dispatch(
                showToast({
                    type: "error",
        ...
        if (["LB", "BY"].includes(payload.runsInput?.label)) {
            if (payload.runsInput?.value <= 0) {
                dispatch(
                    showToast({
                        type: "error",
                        message: "Please select runs"
                    })
                );
                return;
            }
        }
        handleUpdateScore(payload.runsInput?.label, payload).then(() =>
            handleCloseModal()
        );
    };
    ...
    return (
        <Modal
            isVisible={activeModal === "extraRuns"}
                    ...
                    {(payload?.title === "Wide Ball" ||
                        ...
                        <View style={styles.modal_input_wrapper}>
                            ...
                            <Text style={styles.modal_input_sum}>
                                {payload.runsInput?.value === 0 ||
                                isNaN(payload.runsInput?.value)
                                    ? 1
                                    : 1 + Number(payload.runsInput?.value)}
                            </Text>
                        </View>
                    )}
                </View>
