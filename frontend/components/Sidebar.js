...
import { checkAndNavigateToPendingAction } from "../utils/matchUtils.js";
...
const Sidebar = ({ matchDetails, showSidebar, setShowSidebar }) => {
    ...
    const handleLinkPress = link => {
        ...
        if (link === "replaceBatsman") {
            dispatch(openModal({ type: "replaceBatsman" }));
        }
        if (link === "replaceBowler") {
            dispatch(openModal({ type: "replaceBowler" }));
        }
        if (link === "changeSquad") {
            dispatch(openModal({ type: "changeSquad" }));
        }
        if (link === "endInning") {
            dispatch(openModal({ type: "endInning" }));
        }
        if (link === "endMatch") {
            dispatch(openModal({ type: "endMatch" }));
        }
    };
