...
const ChangeSquadScreen = ({
  navigation, route
}) => {
  const [matchDetails,
    setMatchDetails] = useState(null);
  const [teamDetails,
    setTeamDetails] = useState(null);
  const [substitutes,
    setSubstitutes] = useState([]);
  const [restOfSquad,
    setRestOfSquad] = useState([]);
  const [player,
    setPlayer] = useState(null);
  ...
  const handleOpenChangeCaptainModal = player => {
    setPlayer(player);
    dispatch(setChangeCaptainModal({
      isShow: true
    }));
  };
