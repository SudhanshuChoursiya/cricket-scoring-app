...
const RetiredOutAssign = ({
  ...
  const [outEnd,
    setOutEnd] = useState(null);
  const [deliveryType,
    setDeliveryType] = useState(null);
  const [runScored,
    setRunScored] = useState(null);
  const [isLoading,
    setIsLoading] = useState(true);
  const [showSpinner,
    setShowSpinner] = useState(false);
  const [runsInput,
    setRunsInput] = useState( {
      isShow: false, value: ""
    });
  const [isDeadBall,
  ...
  return (
    <View style={styles.wrapper}>
      <View style={styles.header}>
        ...
        <Text style={styles.label}>retired out</Text>
      </View>
      {!isLoading ? (
        <>
...
export default RetiredOutAssign;