...
const SelectCaptain = ({ navigation, route }) => {
    ...
    useEffect(() => {
        const unsubscribe = navigation.addListener("focus", () => {
            if (route.params?.selectFor === "team A") {
                dispatch(setTeamACaptain({ name: null, captainId: null }));
            }
            if (route.params?.selectFor === "team B") {
                dispatch(setTeamBCaptain({ name: null, captainId: null }));
            }
        });
        return unsubscribe;
    }, [navigation]);
    ...
    const selectFor = route.params?.selectFor;
    const teamName =
        selectFor === "team A" ? teamA?.name?.trim() : teamB?.name?.trim();

    const headerText =
        teamName && teamName.length > 0 ? `select ${teamName} captain` : "";
    return (
