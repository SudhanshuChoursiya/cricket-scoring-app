...
const TeamSquadScreen = ({ navigation, route }) => {
    ...
    const { data, isLoading, isError, refetch } = useGetTeamDetailsQuery(
        route.params?.teamId
    );
    ...
    const teamName =
        route.params?.selectFor === "team A" ? teamA?.name : teamB?.name;

    const headingText = teamName ? `select playing 11 ( ${teamName} )` : "";
